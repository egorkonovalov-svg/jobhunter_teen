...
export default function JobFilters() {
  ...
  return (
    <aside className="bg-white border border-border rounded-lg p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xs font-semibold uppercase tracking-wider text-midnight">Фильтры</h2>
        {hasFilters && (
          <button
            onClick={clearAll}
            className="text-xs text-gold hover:text-gold-hover font-medium transition-colors duration-300"
          >
            Сбросить
          </button>
        )}
      ...
      <div>
        ...
        <select
          ...
          className="w-full px-3 py-2.5 bg-white border border-border rounded-lg text-sm text-midnight focus:outline-none focus:border-midnight focus:ring-1 focus:ring-midnight transition-all duration-300"
        >
        ...
        <div className="space-y-2">
          ...
          {currentType && (
            <button
              ...
              className="text-xs text-text-tertiary hover:text-midnight mt-1 transition-colors duration-300"
            >
              Сбросить тип
            </button>
          )}
