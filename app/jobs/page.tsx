  ...
  if (!jobs || jobs.length === 0) {
    return (
      <div className="text-center py-20">
        <p className="heading-display text-6xl text-border mb-4">0</p>
        <h3 className="text-lg font-semibold text-midnight mb-2">Вакансий не найдено</h3>
        <p className="text-text-secondary mb-4">Попробуйте другой город или измените фильтры</p>
        <Link href="/jobs" className="text-gold font-medium hover:text-gold-hover link-underline transition-colors duration-300">
        ...
        <div className="flex items-center justify-center gap-3 mt-8">
          {page > 1 && (
            <Link
              ...
              className="px-4 py-2 border border-border rounded-lg text-sm text-text-secondary hover:border-midnight hover:text-midnight transition-all duration-300"
            >
              ← Назад
            </Link>
          )}
...
export default async function JobsPage({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="heading-display text-3xl font-bold text-midnight mb-8">Вакансии для подростков</h1>
